...
export default function ProductionUpdateForm(props) {
  const {
    id,
    production,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...productionRecord };
    setName(cleanValues.name);
    setExpectedUnits(cleanValues.expectedUnits);
    setExpectedPackages(cleanValues.expectedPackages);
    setExpectedPallets(cleanValues.expectedPallets);
    setStandard(cleanValues.standard);
    setExtraUnits(cleanValues.extraUnits);
    setNotes(cleanValues.notes);
    setErrors({});
  };
  const [productionRecord, setProductionRecord] = React.useState(production);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Production, id) : production;
      setProductionRecord(record);
    };
    queryData();
  }, [id, production]);
  React.useEffect(resetStateValues, [productionRecord]);
  const validations = {
    name: [],
    expectedUnits: [],
    expectedPackages: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          const original = await DataStore.query(Production, id);
      ...
      {...rest}
      {...getOverrideProps(overrides, "ProductionUpdateForm")}
    >
      <TextField
        label="Name"
        isRequired={false}
